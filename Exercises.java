...
public class Exercises {
	...
	public double isItOnSale(String itemNumber) {
		...
		String result = "";
		if(itemNumber != null) {
			result = itemNumber.toUpperCase();
		}
		if(discounts.containsKey(result)) {
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		if(petersMoney > 0 && paulsMoney < 1000) {
			...
			peterPaul.put("Paul", halfOfPetersMoney + paulsMoney);
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		int petersMoney = peterPaul.get("Peter");
		int paulsMoney = peterPaul.get("Paul");
		int peterPaulPartnership = (paulsMoney / 4) + (petersMoney / 4);
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
		...
		Map<String, Integer> smallerWarehouse = new HashMap<>();
		Map<String, Integer> biggerWarehouse = new HashMap<>();
		
		if (mainWarehouse.size() < remoteWarehouse.size()) {
			smallerWarehouse = mainWarehouse;
			biggerWarehouse = remoteWarehouse;
		} else {
			smallerWarehouse = remoteWarehouse;
			biggerWarehouse = mainWarehouse;
		}
		
		for(Map.Entry<String, Integer> sku : smallerWarehouse.entrySet()) {
			if (biggerWarehouse.containsKey(sku.getKey())) {
				biggerWarehouse.put(sku.getKey(), (smallerWarehouse.get(sku.getKey()) + biggerWarehouse.get(sku.getKey())));
			} else {
				biggerWarehouse.put(sku.getKey(), sku.getValue());
			}
		}
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		...
		for (String word : words) {
			...
			String pattern = word.substring(word.length() - 2);
			int patternCounter = 0;
			
			for (int i = 0; i < word.length() - 2; i++) {
				String strToCheckAgainstPattern = word.substring(i, i + 2);
				if(strToCheckAgainstPattern.equals(pattern)) {
					patternCounter++;
				}
			}
			last2RevisitedMap.put(word, patternCounter);
		}
